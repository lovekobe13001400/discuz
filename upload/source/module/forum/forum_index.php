...
//$limit =2;
//$count =4;
//$info = C::t('portal_article_title')->fetch_all_by_sql($where, $orderby,$start,$limit,0);

//1.装修日志（catid=17 取11个）
$diary = C::t('portal_article_title')->fetch_all_by_sql("catid=17","order by aid desc",0,11,0);
$diary_list = array();
foreach ($diary as $key=>$one){
	if($key==0){
		$diary_first_image =  'data/attachment/'.$one['pic'];
		$diary_first_title = $one['title'];
	}else{
		$diary_list[] = array(
			'title'=>$one['title'],
			'aid'=>$one['aid'],
...
//2.完工案例(cateid=15)
$wg_case = C::t('portal_article_title')->fetch_all_by_sql("catid=15","order by aid desc");
$case_loop_arr = array();
foreach ($wg_case as $key=>$one){
	$temp_key = floor($key/10);
	$temp_arr = array(
		'title'=>$one['title'],
		'aid'=>$one['aid'],
		'pic'=>'data/attachment/'.$one['pic']
	);
	//没8个首页多1组展示
	$case_loop_arr[$temp_key][$key] = $temp_arr;

}
//3.工地巡检(cateid=14)
$gdxj = C::t('portal_article_title')->fetch_all_by_sql("catid=14","order by aid desc",0,1);
$gdxj_arr = array();
foreach ($gdxj as $key=>$one){
	$gdxj_arr[] = array(
		'title'=>$one['title'],
		'aid'=>$one['aid'],
		'pic'=>'data/attachment/'.$one['pic']
...
//4.新闻中心(cateid=1)
$news = C::t('portal_article_title')->fetch_all_by_sql("catid=1","order by aid desc",0,15,0);
$news_list = array();
$news_first_image = "";
$news_first_title = "";
foreach ($news as $key=>$one){
	if($key==0){
		$news_first_image =  'data/attachment/'.$one['pic'];
		$news_first_title = $one['title'];
	}else{
		$news_list[] = array(
			'title'=>$one['title'],
			'aid'=>$one['aid'],
...
//5.装修大讲堂(cateid=18)
$jiangtang = C::t('portal_article_title')->fetch_all_by_sql("catid=18","order by aid desc",0,5,0);
$jiangtang_list = array();
foreach ($jiangtang as $key=>$one){
	$jiangtang_list[] = array(
			'pic'=>'data/attachment/'.$one['pic'],
			'title'=>$one['title'],
			'aid'=>$one['aid'],
...
//6.公司团队(cateid=9)
$team = C::t('portal_article_title')->fetch_all_by_sql("catid=9","order by aid desc");
$team_loop_arr = array();
foreach ($team as $key=>$one){
	$temp_key = floor($key/7);
	$temp_arr = array(
		'title'=>$one['title'],
		'aid'=>$one['aid'],
		'pic'=>'data/attachment/'.$one['pic']
	...
	//没8个首页多1组展示
	$team_loop_arr[$temp_key][$key] = $temp_arr;

}
//7.360度视频(cateid=19)
$video360 = C::t('portal_article_title')->fetch_all_by_sql("catid=19","order by aid desc",0,1);
$video360_list = array();
foreach ($video360 as $key=>$one){
	$video360_list[] = array(
		'title'=>$one['title'],
		'aid'=>$one['aid'],
		'pic'=>'data/attachment/'.$one['pic']
...
//8.客户心声(cateid=20)
$client = C::t('portal_article_title')->fetch_all_by_sql("catid=20","order by aid desc",0,1);
$client_list = array();
foreach ($client as $key=>$one){
	$client_list[] = array(
		'title'=>$one['title'],
		'aid'=>$one['aid'],
		'pic'=>'data/attachment/'.$one['pic']
