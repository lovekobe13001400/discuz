...
$cat_where = "catid={$catid}";

//var_dump($cat_list);exit();
//这个类别下的文字
if($catid==14 || $catid==17 ){
	//工地巡检
	$cat_list = category_get_list($cat,$cat_where,1,20);
	$cat_list = $cat_list['list'];
	//var_dump($cat_list);exit();
	include_once template("forum/z_gdxj");
}elseif($catid==5 || $catid==6 || $catid==12 || $catid==13){
	//新闻动态 和学习交流
	$cat_list = category_get_list($cat,$cat_where,1,10);
	$cat_list = $cat_list['list'];
	include_once template("forum/z_new_study");
}elseif($catid==15 || $catid==16){
	//完工案例 装修体验
	$cat_list = category_get_list($cat,$cat_where,1,10);
	$cat_list = $cat_list['list'];
	include_once template("forum/z_example_list");
}else{
	$cat_list = category_get_list($cat,$cat_where,1,12);
	$cat_list = $cat_list['list'];
	include_once template("forum/z_star_shifu");
}

//echo  template('diy:'.$file, NULL, $tpldirectory, NULL, $primaltplname);exit();
